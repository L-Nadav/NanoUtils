...
public class IntSlider extends Slider {
    ...
    public IntSlider(int x, int y, int w, int h, Setting<Integer> s) {
    ...
    public void render() {
        ...
        FontDrawer.drawString(hovering ? String.valueOf(setting.getVal()) : setting.name, screenX+3, screenY+3, fg);
        float pct = MathUtil.percent(0, 1000, milliPct);
        if (setting.step == null)
            setting.setVal((int) MathUtil.fromPercent(setting.minVal, setting.maxVal, pct));
        else
            setting.setVal((int) MathUtil.round(MathUtil.fromPercent(setting.minVal, setting.maxVal, pct), setting.step));
    }
}
